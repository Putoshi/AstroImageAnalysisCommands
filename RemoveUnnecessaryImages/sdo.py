...
# 元素材
IMG_DIR = config['SDO']['IMG_DIR']

# 出力先
OUT_DIR = config['SDO']['OUT_DIR']

# 判定から外れた画像のファイル名を保存するJSON
REMOVE_LIST = config['SDO']['REMOVE_LIST']

# 解析時のイメージサイズ
IMG_SIZE = (
  int(config['SDO']['ANALYSIS_IMG_SIZE']),
  int(config['SDO']['ANALYSIS_IMG_SIZE'])
)

# 解析時のイメージサイズ
HIST_THRESHOLD = float(config['SDO']['HIST_THRESHOLD'])
...
def calcCenter(out_path, img):
  ...
  ret, thresh = cv2.threshold(gauss, 120, 255, cv2.THRESH_BINARY)
